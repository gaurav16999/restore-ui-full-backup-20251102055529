...
const OtherDownloads = () => {
  const sidebarItems = getAdminSidebarItems("/admin/study-material/other-downloads");
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // try to fetch announcements that likely contain downloadable resources
        const res = await authClient.get('/api/admin/announcements/', { params: { search: 'download' } });
  ...
  return (
    <DashboardLayout title="Other Downloads List" userName="Admin" userRole="Administrator" sidebarItems={sidebarItems}>
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>Other Downloads List</CardTitle>
          </CardHeader>
          <CardContent>
