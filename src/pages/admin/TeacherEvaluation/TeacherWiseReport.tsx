...
const TeacherWiseReport = () => {
  const sidebarItems = getAdminSidebarItems("/admin/teacher-evaluation/teacher-wise-report");

  const [teacher, setTeacher] = useState("");
  const [submittedBy, setSubmittedBy] = useState("");

  const handleSearch = () => {
    console.log('search teacher wise evaluation', { teacher, submittedBy });
  };

  const rows: any[] = [];

  return (
    <DashboardLayout title="Teacher Wise Evaluation Report" userName="Admin" userRole="Administrator" sidebarItems={sidebarItems}>
      <div className="space-y-6">
        <Card>
          <CardHeader>
            <CardTitle>Teacher Wise Evaluation Report</CardTitle>
          </CardHeader>
          <CardContent>
