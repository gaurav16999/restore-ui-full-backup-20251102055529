...
class NotificationsView(APIView):
                ...
                formatted_notifications.append(
                    {
                        ...
                        'timestamp': notif.get(
                            ...
                            notif.get(
                                'timestamp',
                                datetime.now())).isoformat() if isinstance(
                            notif.get(
                                'created_at',
                                notif.get('timestamp')),
                            datetime) else notif.get(
                            'created_at',
                            notif.get(
                                'timestamp',
    ...
    def _generate_student_notifications(self, user):
        ...
        if student_class:
            course_count = ClassSubject.objects.filter(
                class_assigned=student_class, is_active=True).count()

        # Count pending assignments from HomeworkSubmission model
        from admin_api.models import Assignment
        pending_assignments = Assignment.objects.filter(
            class_assigned=student_class,
            due_date__gte=timezone.now(),
            is_active=True
        ).exclude(
            homework_submissions__student=student,
            homework_submissions__status__in=['submitted', 'graded']
        ).count()
        ...
        notifications.append(
            {
                ...
                'message': f"Good {time_of_day}, {
                    user.first_name}! You have {course_count} enrolled courses and {pending_assignments} pending assignments.",
                'type': 'success',
                'priority': 'high',
                'actionUrl': '/student',
        ...
        if recent_grades:
            ...
            days_ago = (
                datetime.now().date() -
                latest_grade.created_at.date()).days
            if days_ago <= 1:
                notifications.append(
                    {
                        'title': 'New Grade Posted',
                        'message': f'Your {
                            latest_grade.subject.title if latest_grade.subject else "assignment"} grade has been posted: {
                            latest_grade.score}%',
                        'type': 'success',
                        'priority': 'medium',
                ...
                notifications.append(
                    {
                        ...
                        'message': f'Your attendance is {
                            attendance_rate:.1f}%. Please maintain at least 75% attendance.',
                        'type': 'warning',
                        'priority': 'high',
    ...
    def _generate_teacher_notifications(self, user):
        ...
        classes_count = assigned_classes.values(
            'class_assigned').distinct().count()

        # Get total students across all classes
        student_count = 0
        for assignment in assigned_classes:
            student_count += Student.objects.filter(
                class_name=assignment.class_assigned.name, is_active=True).count()
        ...
        notifications.append(
            {
                ...
                'message': f"Good {time_of_day}, {
                    user.first_name}! You have {classes_count} assigned classes with {student_count} students.",
                'type': 'success',
                'priority': 'high',
                'actionUrl': '/teacher',
                'timestamp': datetime.now()})
    ...
    def _generate_admin_notifications(self, user):
        ...
        notifications.append(
            {
                ...
                'message': f"Good {time_of_day}, {
                    user.first_name}! System overview: {total_students} students, {total_teachers} teachers, {total_classes} classes.",
                'type': 'success',
                'priority': 'high',
                'actionUrl': '/admin',
                'timestamp': datetime.now()})
        ...
        new_students = Student.objects.filter(
            enrollment_date__gte=week_ago).count()
        ...
        unassigned_subjects = ClassSubject.objects.filter(
    ...
    def post(self, request):
        ...
        if action == 'mark_read' and notification_id:
            # Check if this is a temporary ID (dynamically generated
            # notification)
            if isinstance(
                    notification_id,
                    str) and notification_id.startswith('temp_'):
                # Temporary notifications don't exist in database, just return
                # success
                return Response({'status': 'success',
                                 'message': 'Temporary notification acknowledged'})
            ...
            try:
                ...
                # If notification not found, still return success (might have
                # been deleted)
                return Response(
                    {'status': 'success', 'message': 'Notification not found or already deleted'})
            except (ValueError, TypeError):
                # Invalid ID format, return success anyway
                return Response(
                    {'status': 'success', 'message': 'Invalid notification ID'})
