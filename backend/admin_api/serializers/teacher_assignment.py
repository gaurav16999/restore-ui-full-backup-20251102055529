...
class TeacherAssignmentSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # Check if assignment already exists (excluding current instance for updates)
        ...
        if existing.exists():
            raise serializers.ValidationError(
                f"{teacher.get_full_name()} is already assigned to teach {subject.title} in {class_assigned.name}"
            )
