...
class StudentSerializer(serializers.ModelSerializer):
    ...
    dob = serializers.DateField(
        ...
        allow_null=True,
        write_only=True,
        help_text="Alias for date_of_birth")
...
class StudentCreateSerializer(serializers.ModelSerializer):
    ...
    # Make optional for auto-generation
    ...
    def validate_email(self, value):
        ...
        if User.objects.filter(email=value).exists():
            raise serializers.ValidationError(
                f"Email '{value}' is already registered. Please use a different email.")
        return value
    ...
    def create(self, validated_data):
        ...
        if 'roll_number' in validated_data and validated_data.get(
                'roll_number'):
            validated_data['roll_no'] = validated_data.pop('roll_number')
        if 'class_assigned' in validated_data and validated_data.get(
                'class_assigned'):
            validated_data['class_name'] = validated_data.pop('class_assigned')

        # Generate unique ID for both username and roll_no
        if 'roll_no' not in validated_data or not validated_data.get(
                'roll_no'):
