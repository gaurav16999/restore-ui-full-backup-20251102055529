...
class FeePaymentCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FeePayment
        fields = ['student', 'fee_structure', 'amount_due', 'amount_paid', 'payment_method', 
                  'transaction_id', 'status', 'payment_date', 'due_date', 'late_fee', 
                  'discount', 'remarks', 'collected_by']
    
    def create(self, validated_data):
        # Auto-generate invoice number
        import datetime
        prefix = f"INV{datetime.datetime.now().strftime('%Y%m%d')}"
        last_payment = FeePayment.objects.filter(invoice_number__startswith=prefix).order_by('-invoice_number').first()
        if last_payment:
