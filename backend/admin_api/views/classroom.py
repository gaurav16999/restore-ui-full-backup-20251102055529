...
class ClassRoomViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        if old_teacher and old_teacher != new_teacher:
            old_teacher.classes_count = old_teacher.assigned_classrooms.filter(is_active=True).count()
            old_teacher.save()
        
        if new_teacher:
            new_teacher.classes_count = new_teacher.assigned_classrooms.filter(is_active=True).count()
            new_teacher.save()
    ...
    def students(self, request, pk=None):
        ...
        enrollments = classroom.enrollments.filter(is_active=True).select_related('student__user')
    ...
    def toggle_status(self, request, pk=None):
        ...
        if not classroom.is_active:
            classroom.enrollments.filter(is_active=True).update(is_active=False)
    ...
    def stats(self, request):
        ...
        grade_stats = ClassRoom.objects.filter(is_active=True).values('grade_level').annotate(
            count=Count('id')
    ...
    def by_grade(self, request):
        ...
        if grade_level:
            ...
            classrooms = ClassRoom.objects.filter(is_active=True).order_by('grade_level', 'section')
        ...
        if grade_level:
            ...
            grouped_data = []
            for grade, group in groupby(classrooms, key=lambda x: x.grade_level):
