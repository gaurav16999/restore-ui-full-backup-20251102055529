...
class ClassSubjectViewSet(viewsets.ModelViewSet):
    ...
    def by_class(self, request):
        ...
        class_subjects = self.get_queryset().filter(class_assigned_id=class_id, is_active=True)
    ...
    def by_subject(self, request):
        ...
        class_subjects = self.get_queryset().filter(subject_id=subject_id, is_active=True)
        serializer = self.get_serializer(class_subjects, many=True)
        return Response(serializer.data)
    ...
    def stats(self, request):
        ...
        classes_with_subjects = ClassSubject.objects.filter(is_active=True).values('class_assigned').distinct().count()
        subjects_assigned = ClassSubject.objects.filter(is_active=True).values('subject').distinct().count()
        compulsory_count = ClassSubject.objects.filter(is_active=True, is_compulsory=True).count()
