...
from admin_api.models import Student, Class
from admin_api.serializers.student import StudentSerializer, StudentCreateSerializer, StudentImportSerializer
...
class StudentStatsView(generics.RetrieveAPIView):
    def get(self, request):
        ...
        # In a real app, you'd calculate these based on created_at dates and attendance records
        return Response({
            'total': total_students,
            'active': active_students,
            'new_this_month': max(0, total_students // 10),  # Dummy calculation
...
class StudentImportView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if missing_columns:
                return Response(
                    {'error': f'Missing required columns: {", ".join(missing_columns)}. Required: Name, Class. Optional: Age, Email'}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
                ...
                for index, row in df.iterrows():
                    try:
                        ...
                        if not class_name or class_name.lower() == 'nan':
                            errors.append(f'Row {index + 2}: Class is required')
                            continue
...
class StudentCredentialsDownloadView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Create CSV writer
            writer = csv.DictWriter(response, fieldnames=['Student ID', 'Name', 'Class', 'Email', 'Password'])
            writer.writeheader()
